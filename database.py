...
class Database:
    ...
    def get_user_giveaways(self, user_id):
        with self._conn() as conn:
            rows = conn.execute("SELECT * FROM giveaways WHERE creator_id=? AND status='active' ORDER BY created_at DESC", (user_id,)).fetchall()
            result = []
            for r in rows:
                d = dict(r)
                try: d["tg_channels"] = json.loads(d.get("tg_channels") or "[]")
    ...
    def is_participant(self, gid, user_id):
        with self._conn() as conn:
            return conn.execute("SELECT 1 FROM participants WHERE giveaway_id=? AND user_id=?", (gid, user_id)).fetchone() is not None

    def add_participant(self, gid, user_id, username, profile_link=""):
        try:
            with self._conn() as conn:
                conn.execute("INSERT INTO participants (giveaway_id,user_id,username,profile_link,joined_at) VALUES (?,?,?,?,?)",
    ...
    def get_participants(self, gid):
        with self._conn() as conn:
            return [dict(r) for r in conn.execute("SELECT * FROM participants WHERE giveaway_id=? ORDER BY joined_at ASC", (gid,)).fetchall()]

    def get_participant_count(self, gid):
        with self._conn() as conn:
            return conn.execute("SELECT COUNT(*) FROM participants WHERE giveaway_id=?", (gid,)).fetchone()[0]

    def get_participant_by_id(self, gid, user_id):
        with self._conn() as conn:
            row = conn.execute("SELECT * FROM participants WHERE giveaway_id=? AND user_id=?", (gid, user_id)).fetchone()
    ...
    def update_giveaway_message(self, gid, chat_id, message_id):
        with self._conn() as conn:
            conn.execute("UPDATE giveaways SET post_chat_id=?, post_message_id=? WHERE id=?", (chat_id, message_id, gid))

    def get_giveaway_post(self, gid):
        with self._conn() as conn:
            row = conn.execute("SELECT post_chat_id, post_message_id FROM giveaways WHERE id=?", (gid,)).fetchone()
    ...
    def finish_giveaway(self, gid, winner_ids):
        with self._conn() as conn:
            ...
            for uid in winner_ids:
                conn.execute("INSERT INTO winners (giveaway_id,user_id,chosen_at) VALUES (?,?,?)", (gid, uid, datetime.now().isoformat()))
    ...
    def set_captcha(self, user_id, gid, answer):
        with self._conn() as conn:
            conn.execute("INSERT OR REPLACE INTO captcha_sessions (user_id,giveaway_id,answer,created_at) VALUES (?,?,?,?)",
                         (user_id, gid, answer, datetime.now().isoformat()))

    def get_captcha(self, user_id, gid):
        with self._conn() as conn:
            row = conn.execute("SELECT answer FROM captcha_sessions WHERE user_id=? AND giveaway_id=?", (user_id, gid)).fetchone()
            return row["answer"] if row else None

    def clear_captcha(self, user_id, gid):
        with self._conn() as conn:
            conn.execute("DELETE FROM captcha_sessions WHERE user_id=? AND giveaway_id=?", (user_id, gid))
